...
# Description: A multi-analog-mode receiver for UHD and RTLSDR for Linux/OSX
# Generated: Wed Apr  8 10:41:29 2015
##################################################
...
class multimode(grc_wxgui.top_block_gui):
    ...
    def __init__(self, ahw="default", freq=150.0e6, ppm=0.0, vol=1.0, ftune=0.0, xftune=0.0, srate=1.0e6, devinfo="rtl=0", agc=0, arate=48.0e3, mthresh=-10.0, flist="", dmode="NFM1", dfifo="multimode_fifo", aout="/dev/null", upce=0, upclo=0.0, mbw=2.0e3, deemph=75.0e-6):
        ...
        self.samp_rate_0 = samp_rate_0 = int(mh.get_good_rate(devinfo,srate))
        self.rfgain = rfgain = 10
        self.recfn = recfn = "/dev/null" if record == False else record_file
        self.probe_mult = probe_mult = 1.0/math.sqrt(mh.get_mode_deviation(mode,bw))*250.0
        ...
        self.digifn = digifn = "/dev/null" if mh.get_mode_type(mode) != "DIG" else dfifo
        ...
        # Blocks
        ##################################################
        self.rf_probe = analog.probe_avg_mag_sqrd_c(0, 0.015)
        ...
        self.display_probe = analog.probe_avg_mag_sqrd_c(0, 0.002)
        ...
        self.osmosdr_source_0_0.set_center_freq(cur_freq+(upc_offset+float(upc))+fine+xfine, 0)
        ...
        self.osmosdr_source_0_0.set_gain(5 if iagc == 1 else rfgain, 0)
        self.osmosdr_source_0_0.set_if_gain(10, 0)
        self.osmosdr_source_0_0.set_bb_gain(8, 0)
        ...
        self.gr_multiply_const_vxx_0 = blocks.multiply_const_vcc((complex(probe_mult), ))
        self.gr_keep_one_in_n_1 = blocks.keep_one_in_n(gr.sizeof_gr_complex*1, aratio)
        self.gr_keep_one_in_n_0 = blocks.keep_one_in_n(gr.sizeof_gr_complex*1, int(wbfm/digi_rate))
        self.gr_fft_filter_xxx_3 = filter.fft_filter_ccc(1, (zoom_taps), 1)
        self.gr_fft_filter_xxx_3.declare_sample_delay(0)
        self.gr_fft_filter_xxx_2_0 = filter.fft_filter_fff(5, (firdes.low_pass(1.0,wbfm,14.5e3,8.5e3,firdes.WIN_HAMMING,6.76)), 1)
        self.gr_fft_filter_xxx_2_0.declare_sample_delay(0)
        self.gr_fft_filter_xxx_2 = filter.fft_filter_ccc(1, (main_taps), 1)
        self.gr_fft_filter_xxx_2.declare_sample_delay(0)
        self.gr_fft_filter_xxx_0 = filter.fft_filter_ccc(int(samp_rate/wbfm), (firdes.low_pass(1.0,samp_rate,98.5e3,66e3,firdes.WIN_HAMMING,6.76)), 1)
        self.gr_fft_filter_xxx_0.declare_sample_delay(0)
        self.gr_feedforward_agc_cc_0 = analog.feedforward_agc_cc(1024, 0.75)
        self.fractional_resampler_xx_0 = filter.fractional_resampler_ff(0, audio_int_rate/arate)
        ...
        self.blocks_wavfile_sink_0 = blocks.wavfile_sink(recfn, 1, int(audio_int_rate), 8)
        self.blocks_multiply_const_vxx_3 = blocks.multiply_const_vff((0.0 if muted else volume/4.5, ))
        self.blocks_multiply_const_vxx_2 = blocks.multiply_const_vff((0.55 if mh.get_mode_type(mode) == "AM" else 0.0, ))
        self.blocks_multiply_const_vxx_1 = blocks.multiply_const_vff((0.85 if mh.get_mode_type(mode) == "SSB" else 0.0, ))
        self.blocks_multiply_const_vxx_0 = blocks.multiply_const_vff((1.0 if mh.get_mode_type(mode) == "FM" else 0.0, ))
        self.blocks_keep_one_in_n_0 = blocks.keep_one_in_n(gr.sizeof_gr_complex*1, 1)
        self.blocks_file_sink_1 = blocks.file_sink(gr.sizeof_gr_complex*1, digifn, False)
        self.blocks_file_sink_1.set_unbuffered(True)
        self.blocks_complex_to_real_0 = blocks.complex_to_real(1)
        self.blocks_complex_to_mag_squared_0 = blocks.complex_to_mag_squared(1)
        self.blocks_add_xx_0 = blocks.add_vff(1)
        self.audio_sink_0 = audio.sink(int(arate), "", True)
        self.analog_quadrature_demod_cf_0 = analog.quadrature_demod_cf(k)
        self.analog_fm_deemph_0 = analog.fm_deemph(fs=audio_int_rate, tau=deemph)

        ##################################################
        # Connections
        ##################################################
        self.connect((self.analog_fm_deemph_0, 0), (self.blocks_multiply_const_vxx_0, 0))    
        self.connect((self.analog_quadrature_demod_cf_0, 0), (self.gr_fft_filter_xxx_2_0, 0))    
        ...
        self.connect((self.blocks_keep_one_in_n_0, 0), (self.wxgui_fftsink2_0, 0))    
        self.connect((self.blocks_keep_one_in_n_0, 0), (self.wxgui_waterfallsink2_0, 0))    
        ...
        self.connect((self.blocks_multiply_const_vxx_3, 0), (self.audio_sink_0, 1))    
        self.connect((self.blocks_multiply_const_vxx_3, 0), (self.blocks_wavfile_sink_0, 0))    
        self.connect((self.fractional_resampler_xx_0, 0), (self.blocks_multiply_const_vxx_3, 0))    
        self.connect((self.gr_feedforward_agc_cc_0, 0), (self.blocks_complex_to_mag_squared_0, 0))    
        self.connect((self.gr_feedforward_agc_cc_0, 0), (self.blocks_complex_to_real_0, 0))    
        self.connect((self.gr_fft_filter_xxx_0, 0), (self.gr_fft_filter_xxx_2, 0))    
        self.connect((self.gr_fft_filter_xxx_2, 0), (self.analog_quadrature_demod_cf_0, 0))    
        ...
        self.connect((self.gr_fft_filter_xxx_2_0, 0), (self.analog_fm_deemph_0, 0))    
        self.connect((self.gr_fft_filter_xxx_3, 0), (self.blocks_keep_one_in_n_0, 0))    
        self.connect((self.gr_keep_one_in_n_0, 0), (self.blocks_file_sink_1, 0))    
        ...
        self.connect((self.osmosdr_source_0_0, 0), (self.gr_fft_filter_xxx_0, 0))    
        self.connect((self.osmosdr_source_0_0, 0), (self.gr_fft_filter_xxx_3, 0))    
    ...
    def set_srate(self, srate):
        self.srate = srate
        self.set_samp_rate_0(int(mh.get_good_rate(self.devinfo,self.srate)))
    ...
    def set_arate(self, arate):
        self.arate = arate
        self.fractional_resampler_xx_0.set_resamp_ratio(self.audio_int_rate/self.arate)
    ...
    def set_dfifo(self, dfifo):
        self.dfifo = dfifo
        self.set_digifn("/dev/null" if mh.get_mode_type(self.mode) != "DIG" else self.dfifo)
    ...
    def set_record_file(self, record_file):
        ...
        self.set_recfn("/dev/null" if self.record == False else self.record_file)
    ...
    def set_mode(self, mode):
        ...
        self.set_digifn("/dev/null" if mh.get_mode_type(self.mode) != "DIG" else self.dfifo)
        self.set_k(self.wbfm/(2*math.pi*mh.get_mode_deviation(self.mode,self.bw)))
        self.set_probe_mult(1.0/math.sqrt(mh.get_mode_deviation(self.mode,self.bw))*250.0)
        self._mode_chooser.set_value(self.mode)
        self.blocks_multiply_const_vxx_0.set_k((1.0 if mh.get_mode_type(self.mode) == "FM" else 0.0, ))
        self.blocks_multiply_const_vxx_2.set_k((0.55 if mh.get_mode_type(self.mode) == "AM" else 0.0, ))
        self.blocks_multiply_const_vxx_1.set_k((0.85 if mh.get_mode_type(self.mode) == "SSB" else 0.0, ))
    ...
    def set_cur_freq(self, cur_freq):
        ...
        self.osmosdr_source_0_0.set_center_freq(self.cur_freq+(self.upc_offset+float(self.upc))+self.fine+self.xfine, 0)
    ...
    def set_volume(self, volume):
        ...
        self.blocks_multiply_const_vxx_3.set_k((0.0 if self.muted else self.volume/4.5, ))
    ...
    def set_rfgain(self, rfgain):
        ...
        self.osmosdr_source_0_0.set_gain(5 if self.iagc == 1 else self.rfgain, 0)

    def get_recfn(self):
        return self.recfn

    def set_recfn(self, recfn):
        self.recfn = recfn
        self.blocks_wavfile_sink_0.open(self.recfn)

    def get_probe_mult(self):
        return self.probe_mult

    def set_probe_mult(self, probe_mult):
        self.probe_mult = probe_mult
        self.gr_multiply_const_vxx_0.set_k((complex(self.probe_mult), ))
    ...
    def set_k(self, k):
        self.k = k
        self.analog_quadrature_demod_cf_0.set_gain(self.k)
    ...
    def set_digifn(self, digifn):
        self.digifn = digifn
        self.blocks_file_sink_1.open(self.digifn)
...
if __name__ == '__main__':
    ...
    parser.add_option("", "--aout", dest="aout", type="string", default="/dev/null",
        help="Set Output file for mono audio output [default=%default]")
    ...
    tb = multimode(ahw=options.ahw, freq=options.freq, ppm=options.ppm, vol=options.vol, ftune=options.ftune, xftune=options.xftune, srate=options.srate, devinfo=options.devinfo, agc=options.agc, arate=options.arate, mthresh=options.mthresh, flist=options.flist, dmode=options.dmode, dfifo=options.dfifo, aout=options.aout, upce=options.upce, upclo=options.upclo, mbw=options.mbw, deemph=options.deemph)
    tb.Run(True, 300)
